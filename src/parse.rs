...
impl Time {
    pub fn new(hour: u16, minute: u16) -> Result<Time, ParseFailure> {
        if !is_valid_time(hour, minute) {
            Err(ParseFailure::from_str("time", "not a valid time"))
        } else {
    ...
    pub fn parse(text: &str) -> Result<Time, ParseFailure> {
...
impl Date {
    pub fn new(year: u16, month: u16, day: u16) -> Result<Date, ParseFailure> {
        if !is_valid_date(year, month, day) {
            Err(ParseFailure::from_str("date", "not a valid date"))
        } else {
            Ok(Date { year, month, day })
        }
    }

    pub fn parse(text: &str) -> Result<Date, ParseFailure> {
...
impl DayEntry {
    ...
    fn add_label(&mut self, project_times: ProjectTimes) {
...
fn parse_digits(context: &str, re: &Regex, text: &str, group: usize) -> Result<u16, ParseFailure> {
    let digit_str = re
        .captures(text)
        .and_then(|m| m.get(group))
        .map(|m| m.as_str())
        .ok_or_else(|| {
            ParseFailure::from_str(context, format!("cannot find value in {text}").as_str())
        })?;
    let number = digit_str
        .parse::<u16>()
        .map_err(|e| ParseFailure::from_error(context, e))?;
    Ok(number)
}
...
fn parse_time(hhmm: &str) -> Result<Time, ParseFailure> {
    Time::parse(hhmm)
}

fn parse_time_range(text: &str) -> Result<TimeRange, ParseFailure> {
    let caps = TIME_RANGE_RE
        .captures(text)
        .ok_or_else(|| ParseFailure::from_str("time range", "not a time range"))?;
...
fn parse_time_ranges(time_range_str: &str) -> Result<Vector<TimeRange>, ParseFailure> {
...
fn parse_date_line(line: &str) -> Result<Date, ParseFailure> {
    let caps = DATE_LINE_RE
        .captures(line)
        .ok_or_else(|| ParseFailure::from_str("date line", "not a date line"))?;
    Date::parse(caps[1].to_string().as_str())
}

// Function to parse label and time ranges (e.g., "client,project: 0800-1200,1300-1310")
fn parse_time_line(line: &str) -> Result<ProjectTimes, ParseFailure> {
    let caps = TIME_LINE_RE
        .captures(line)
        .ok_or_else(|| ParseFailure::from_str("time line", "not a time line"))?;
...
pub fn parse_file(file_path: &str) -> Result<Vector<DayEntry>, ParseFailure> {
    let mut day_entries = Vector::new();
    let path = Path::new(file_path);
    let file = File::open(path).map_err(|e| ParseFailure::from_error("i/o", e))?;
    ...
    for raw_line in reader.lines() {
        let line = raw_line
            ...
            .map_err(|e| ParseFailure::from_error("i/o", e))?;
...
mod tests {
    ...
    fn time_range(h1: u16, m1: u16, h2: u16, m2: u16) -> TimeRange {
        TimeRange {
            from: Time::new(h1, m1).unwrap(),
            to: Time::new(h2, m2).unwrap(),
        }
    }
    ...
    fn test_parse_file() {
        ...
        let expected = vector![DayEntry {
            date: Date::new(2025, 4, 3).unwrap(),
            projects: vector![
                ProjectTimes {
                    client: "abc".to_string(),
                    project: "xyz".to_string(),
                ...
                ProjectTimes {
                    ...
                    project: "uvw".to_string(),
                    time_ranges: vector![time_range(12, 0, 13, 0),],
                },
            ],
        }];
